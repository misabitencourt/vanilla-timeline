...
function findDuplicate(list, item) {
    return !! list.find((i) => i !== item && i.id === item.id)
}
...
export default function(data) {
    ...
    if (! (data.booking && data.booking.length)) {
        ...
        data.booking.forEach((booking) => {
            ...
            if (! isValidIsoDate(booking.start)) {
                errors.push('Booking with invalid start date!!')
            }
            if (! isValidIsoDate(booking.end)) {
                errors.push('Booking with invalid end date!!')
            }
            if (! booking.subject) {
